...
class ToolCallInstrumentationTestCase(TestCase):
    ...
    def test_tool_calls_record_parameter_values_on_span_if_enabled(self):
        ...
        arguments = json.loads(
            generated_span.attributes["gen_ai.tool.call.arguments"]
        )
        self.assertEqual(
            arguments["code.function.parameters.someparam.type"],
            "int",
        )
        self.assertEqual(
            arguments["code.function.parameters.otherparam.type"],
            "str",
        )
        self.assertEqual(
            arguments["code.function.parameters.someparam.value"],
            123,
        )
        self.assertEqual(
            arguments["code.function.parameters.otherparam.value"],
            "abc",
        )
    ...
    def test_tool_calls_do_not_record_return_values_if_not_enabled(self):
        ...
        self.otel.assert_has_span_named("execute_tool somefunction")
        generated_span = self.otel.get_span_named("execute_tool somefunction")
        self.assertNotIn("gen_ai.tool.call.result", generated_span.attributes)
